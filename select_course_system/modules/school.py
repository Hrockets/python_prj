...
class School(object):
    ...
    def show_course(self):
        ...
        for key in self.school_course:
            ...
            print('\033[32:1m 课程：%s\t 价格：%s\t 周期：%s月\033[0m' %(course_obj.course_name,
    ...
    def show_class(self):
        ...
        for key in self.school_class:
            ...
            print('\033[32:1m 班级：%s\t 关联课程：%s\t\033[0m' % (class_obj.class_name,
                                                                course_obj.course_name))
    def show_class_course(self):
        for key in self.school_class:
            class_obj = self.school_class[key]
            course_obj = class_obj.class_course
            print('\033[32:1m 班级：%s\t 关联课程：%s\t价格：%s\t 周期：%s月\033[0m' % (class_obj.class_name,
                                                                course_obj.course_name,
                                                                course_obj.course_price,
                                                                course_obj.course_time))
    ...
    def show_teacher(self):
        ...
        for key in self.school_teacher:
            ...
            for i in teacher_obj.teacher_class:
                class_list.append(i)
            print('\033[32:1m 教师：%s\t 薪资：%s\t关联班级：%s\t\033[0m' % (teacher_obj.teacher_name,
    ...
    def show_teacher_classinfo(self,teacher_name):
        teacher_obj = self.school_teacher[teacher_name]
        for i in teacher_obj.teacher_class:
            class_obj = teacher_obj.teacher_class[i]
            student_list =[]
            for k in  class_obj.class_student:
                student_list.append(k)
            print('\033[32:1m 班级：%s\t 关联课程：%s\t学员：%s\t\033[0m' % (class_obj.class_name,
